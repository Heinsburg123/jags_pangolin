...
class Sample_prob:
    ...
    def sample(self, sample_vars: list[RV], kwargs=[], values=[], niter=1000, debug=False):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Write data.R with full float64 precision via write_constant()
            # Previously used Scalar_ops.Constant_before which truncated to ~6 sig figs
            with open(data_path, "w") as f:
                for node in res:
                    if res[node].op.name == "Constant":
                        write_constant(f, node, res[node].op.value)
                for i in range(len(kwargs)):
                    write_constant(f, f"v{kwargs[i]._n}", values[i])
